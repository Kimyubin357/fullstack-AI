...
router.post('/create',upload.single('file'),async(req,res)=>{
    ...
    //스텝4: 신규 등록된 게시글의 고유번호를 기반으로 첨부파일 등록 처리를 한다.
    if(uploadFile){
        //업로드한 파일이 있는 경우
        ...
        //file 첨부데이터를 article_file 테이블에 저장하기
        await db.AritcleFile.create(file)
    }
...
router.get('/modify/:id',async(req,res)=>{
    ...
    //해당 게시글 첨부파일 정보조회
    const articleFile = await db.AritcleFile.findOne({where:{article_id:articleIdx}})
